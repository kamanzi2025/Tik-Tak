/**
 * Grab & Go — Seed Script  (run ONCE from the project root)
 * ==========================================================
 * Prerequisites:
 *   1. npm install firebase   (in this directory, or globally)
 *   2. Replace FIREBASE_CONFIG below with your real credentials
 ...
 *
 * Creates:
 *   • 5 restaurants in Firestore
 *   • 5–6 menu items per restaurant
 *   • 1 staff Firebase Auth user + Firestore doc per restaurant
...
const MENU_ITEMS = {
  'mama-africa': [
    ...
    { name: 'Akabanga Chicken', category: 'Grills', price: 3000, description: 'Quarter chicken with spicy akabanga marinade', inStock: true },
  ...
  'pizza-corner': [
    ...
    { name: 'Chicken Wrap', category: 'Snacks', price: 2800, description: 'Grilled chicken, avocado, coleslaw in a soft wrap', inStock: false },
  ...
  'cafe-latte': [
    ...
    { name: 'Chocolate Croissant', category: 'Pastries', price: 1200, description: 'Buttery croissant filled with dark chocolate', inStock: true },
    { name: 'Blueberry Muffin', category: 'Pastries', price: 1000, description: 'Moist muffin packed with fresh blueberries', inStock: true },
    { name: 'Iced Matcha Latte', category: 'Cold Drinks', price: 2000, description: 'Ceremonial matcha with oat milk over ice', inStock: true },
    { name: 'Banana Bread Slice', category: 'Pastries', price: 900, description: 'Moist banana bread with a hint of cinnamon', inStock: true },
  ],
  'green-bowl': [
    { name: 'Caesar Salad', category: 'Salads', price: 2500, description: 'Romaine, croutons, parmesan, Caesar dressing', inStock: true },
    ...
    { name: 'Hummus & Veggie Wrap', category: 'Wraps', price: 2200, description: 'Hummus, roasted peppers, cucumber, spinach in whole wheat wrap', inStock: true },
    { name: 'Fresh Orange Juice', category: 'Drinks', price: 1200, description: 'Cold-pressed orange juice, no sugar added', inStock: true },
  ],
  'spice-house': [
    { name: 'Butter Chicken', category: 'Curries', price: 3500, description: 'Creamy tomato-based curry with tender chicken', inStock: true },
    ...
    { name: 'Pad Thai Noodles', category: 'Noodles', price: 3000, description: 'Stir-fried rice noodles with egg, bean sprouts, peanuts', inStock: true },
...
async function createAuthUser(email, password, firestoreData) {
  try {
...
async function seed() {
  console.log('\n🌱  Grab & Go seed starting…\n')

  for (const restaurant of RESTAURANTS) {
    const { staff, id, ...data } = restaurant
    console.log(`📍 ${restaurant.name}`)

    await setDoc(doc(db, 'restaurants', id), { ...data, createdAt: serverTimestamp() })

    console.log(`  👤 Creating staff: ${staff.email}`)
    await createAuthUser(staff.email, staff.password, {
    ...
    for (const item of items) {
      await addDoc(collection(db, 'menuItems'), {
        ...item, restaurantId: id, imageUrl: '', createdAt: serverTimestamp(),
      })
    }
    console.log(`  ✅ Done\n`)
  }
  ...
  await createAuthUser(DEMO_CUSTOMER.email, DEMO_CUSTOMER.password, {
  ...
  RESTAURANTS.forEach((r) => {
    console.log(`  ${r.name.padEnd(22)} ${r.staff.email} / ${r.staff.password}`)
  })
  console.log('═══════════════════════════════════════════════════\n')
  process.exit(0)
}
