...
import {
  localRegisterStaff, localLoginStaff, localLogoutStaff,
} from './localStore'

export async function registerStaff(restaurantName, email, password) {
  if (!isFirebaseConfigured) return localRegisterStaff(restaurantName, email, password)
...
export async function loginStaff(email, password) {
  if (!isFirebaseConfigured) return localLoginStaff(email, password)
...
export async function logoutStaff() {
  if (!isFirebaseConfigured) return localLogoutStaff()
  await signOut(auth)
}
