...
export default function Profile() {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="bg-customer px-4 pt-12 pb-8">
        <div className="flex flex-col items-center">
          <div className="w-20 h-20 rounded-full bg-white flex items-center justify-center shadow">
            <span className="text-customer font-bold text-2xl">{initials}</span>
          </div>
          {editing ? (
            <div className="mt-3 flex gap-2 w-full max-w-[200px]">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="bg-white text-customer rounded-lg px-3 py-1.5 text-xs font-bold"
              >{saving ? '…' : 'Save'}</button>
            </div>
          ) : (
            <>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 pb-24 space-y-4">
        <div className="bg-white rounded-2xl shadow-sm divide-y divide-gray-100">
          ...
          <div className="px-4 py-3 flex justify-between items-center">
            ...
            <span className="text-sm font-medium text-gray-900">Campus School</span>
          </div>
          <div className="px-4 py-3 flex justify-between items-center">
        ...
        <button
          ...
          disabled={signingOut}
          className="w-full bg-red-50 text-red-600 rounded-2xl py-3.5 font-semibold text-sm border border-red-100 active:bg-red-100"
        >
