...
export default function Sales() {
  ...
  const stats = useMemo(() => {
    ...
    return { total: filtered.length, revenue, completed: completed.length, declined: declined.length, topItems }
  }, [filtered])
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-xs font-semibold border transition-colors ${
                filter === f ? 'bg-staff text-white border-staff' : 'bg-white text-gray-600 border-gray-200'
              }`}
            >
              {f}
            </button>
                  ...
                  <div key={order.id} className="px-4 py-3 border-b border-gray-100 last:border-0">
                    <div className="flex justify-between items-start">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900">#{shortId} · {order.customerName}</p>
