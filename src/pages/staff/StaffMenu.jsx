...
const PlusIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" className="w-5 h-5">
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="bg-staff px-4 pt-12 pb-4">
        <p className="text-white/60 text-xs">Menu Management</p>
      ...
      <div className="flex-1 overflow-y-auto pb-24 px-4 py-4">
        {grouped.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-sm mt-2">No items yet. Add your first item!</p>
          </div>
        ) : (
          <div className="space-y-4">
            {grouped.map(({ cat, items: catItems }) => (
              <div key={cat} className="bg-white rounded-2xl shadow-sm overflow-hidden">
                ...
                <div className="px-4">
                  {catItems.map((item) => (
                    <MenuItemRow key={item.id} item={item} onEdit={setModalItem} />
                  ))}
                </div>
              </div>
            ))}
