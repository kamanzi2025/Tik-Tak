  ...
  return (
    ...
    >
      ...
      <div onClick={(e) => e.stopPropagation()}>
        <button
          onClick={toggleStock}
          className={`relative w-11 h-6 rounded-full transition-colors ${item.inStock ? 'bg-customer' : 'bg-gray-300'}`}
        >
          <span
            className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${item.inStock ? 'translate-x-5' : 'translate-x-0.5'}`}
          />
        </button>
      </div>
    </div>
